...
export class TurnaroundFilter implements Filter {
    ...
    apply(context: QueryContext): void {
        ...
        if (this.payload.main !== undefined) {
            context.where.push(`g.turnarounds_main ${resolveQuantityComparison(this.payload)} ${this.payload.main}`);
        } else if (this.payload.opponent !== undefined) {
            context.where.push(`g.turnarounds_opponent ${resolveQuantityComparison(this.payload)} ${this.payload.main}`);
        } else if (this.payload.total !== undefined) {
            context.where.push(`(g.turnarounds_main + g.turnarounds_opponent) ${resolveQuantityComparison(this.payload)} ${this.payload.total}`);
        } else {
